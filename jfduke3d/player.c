...
long diedtime;
const long forcedrespawntime = 10;
    ...
    else switch(cw)
    {
        ...
        case RPG_WEAPON:
            ...
            if(*kb > 0)
            {
                if(*kb < 8)
                {
                    myospal(widescreen_weapon_offset(weapon_xoffset+164),(looking_arc<<1)+176-gun_pos,
        ...
        case CHAINGUN_WEAPON:
         ...
         case PISTOL_WEAPON:
            ...
            {
                if((*kb) < 10)
                    myospal(widescreen_weapon_offset(194-(p->look_ang>>1)),looking_arc+230-gun_pos,FIRSTGUN+4,gs,o,pal);
                else if((*kb) < 15)
                {
                    myospal(widescreen_weapon_offset(244-((*kb)<<3)-(p->look_ang>>1)),looking_arc+130-gun_pos+((*kb)<<4),FIRSTGUN+6,gs,o,pal);
                    myospal(widescreen_weapon_offset(224-(p->look_ang>>1)),looking_arc+220-gun_pos,FIRSTGUN+5,gs,o,pal);
                }
                else if((*kb) < 20)
                {
                    myospal(widescreen_weapon_offset(124+((*kb)<<1)-(p->look_ang>>1)),looking_arc+430-gun_pos-((*kb)<<3),FIRSTGUN+6,gs,o,pal);
                    myospal(widescreen_weapon_offset(224-(p->look_ang>>1)),looking_arc+220-gun_pos,FIRSTGUN+5,gs,o,pal);
                }
				else if (NAM)
				{
#define WEAPON2_RELOAD_TIME 50
					...
					{
						myospal(widescreen_weapon_offset(184-(p->look_ang>>1)),looking_arc+235-gun_pos,FIRSTGUN+8,gs,o,pal);
						myospal(widescreen_weapon_offset(224-(p->look_ang>>1)),looking_arc+210-gun_pos,FIRSTGUN+5,gs,o,pal);
					}
					else if((*kb) < (WEAPON2_RELOAD_TIME - 6))
					{
						myospal(widescreen_weapon_offset(164-(p->look_ang>>1)),looking_arc+245-gun_pos,FIRSTGUN+8,gs,o,pal);
						myospal(widescreen_weapon_offset(224-(p->look_ang>>1)),looking_arc+220-gun_pos,FIRSTGUN+5,gs,o,pal);
					}
					else if((*kb) < (WEAPON2_RELOAD_TIME) )
						myospal(widescreen_weapon_offset(194-(p->look_ang>>1)),looking_arc+235-gun_pos,FIRSTGUN+5,gs,o,pal);
				}
                else if((*kb) < 23)
                {
                    myospal(widescreen_weapon_offset(184-(p->look_ang>>1)),looking_arc+235-gun_pos,FIRSTGUN+8,gs,o,pal);
        ...
        case FREEZE_WEAPON:
            ...
            if((*kb))
            {
                ...
                gun_pos -= 16;
                myospal(widescreen_weapon_offset(weapon_xoffset+210-(p->look_ang>>1)),looking_arc+261-gun_pos,FREEZE+2,-32,o,pal);
                myospal(widescreen_weapon_offset(weapon_xoffset+210-(p->look_ang>>1)),looking_arc+235-gun_pos,FREEZE+3+cat_frames[*kb%6],-32,o,pal);
            }
            else myospal(widescreen_weapon_offset(weapon_xoffset+210-(p->look_ang>>1)),looking_arc+261-gun_pos,FREEZE,gs,o,pal);
...
void ForceRespawn(struct player_struct *p, short snum) {
	if (snum == myconnectindex && ud.multimode > 1 && ud.coop == 0) {
		if(p->last_extra == 0) {
			if (diedtime + 26*forcedrespawntime < p->player_par) {
				loc.bits |= (1<<29); //use/open
			}
		} else {
			diedtime = p->player_par;
		}
	}
}
...
void processinput(short snum)
{
	...
	if (ud.timelimit > 0) {
        ...
        if (ticks - ud.matchtime > ud.timelimit) {
            if (myconnectindex == connecthead) {
                sendstop(SC_TIMELIMIT);
                gameexit(" ");
            }
        }
        
        if (ticks - ud.matchtime > ud.timelimit + 5000) { /* backup plan for the case with missed timelimit notification packet */
            if (myconnectindex != connecthead) {
                //printf("[CLIENT] timelimit hit (automatically, server havent notified us)\n");
                //dnQuitToTitle("");
                gameexit(" ");
            }
        }
	...
	if( s->extra <= 0 )
	{
		if(p->dead_flag == 0)
		{
...
#if 0
			// Close and save the demo for this session on death
			// TODO_MP
			if (ud.recstat == 1 && dnIsSinglePlayer())
			{
				extern void StopRecording(int ForDeath);
				StopRecording(1);
			}
#endif
			...
			{
				//DMP_ScoreFrag( p->frag_ps, snum );
				if (myconnectindex == p->frag_ps && ud.recstat != 2)
				{
					//DMP_RecordRankedKill();
				}
				...
				if(p->frag_ps != snum)
				{
					...
					if (p->frag_ps == myconnectindex) {
                        dnRecordMultiplayerKill();
                    }
                    
                    if ((ps[p->frag_ps].frag - ps[p->frag_ps].fraggedself) >= ud.fraglimit && ud.fraglimit != 0) {
                        /* fraglimit hit */
                        gameexit(" ");
                        return;
                    }
					...
					if( ud.user_name[p->frag_ps][0] != 0)
					{
						...
						{
#if FTAHACK
							strcpy(g_FTAExtraStr, &ud.user_name[p->frag_ps][0]);
							g_FTAHackMode = 0;
#endif
						...
						{
#if FTAHACK
							strcpy(g_FTAExtraStr, &ud.user_name[snum][0]);
							g_FTAHackMode = 0;
#endif
					...
					else
					{
						...
						{
#if FTAHACK
							sprintf(g_FTAExtraStr, "%d", 1+p->frag_ps);
							g_FTAHackMode = 1;
#endif
						...
						{
#if FTAHACK
							sprintf(g_FTAExtraStr, "%d", 1+snum);
							g_FTAHackMode = 1;
#endif
		...
		// PLK - Need to keep pumping the time
		if (ud.multimode > 1)
		{
			p->player_par++;
		}
	...
	if (ps[snum].tipincs == 1 && ud.recstat != 2 /* && !dnHasTakenOverDemo() && !dnFastForwardOnDeath() */)
	{
...
HORIZONLY:
...
#if 0		// We steal these bits on XBox
	if( sb_snum&(1<<13) )
	{
		p->return_to_center = 9;
	...
	if(p->knee_incs > 0)
	{
		...
		if(p->knee_incs > 15)
		{
			...
			if(p->actorsqu >= 0 && sprite[p->actorsqu].statnum != MAXSTATUS && dist(&sprite[pi],&sprite[p->actorsqu]) < 1400 )
			{
		...
		switch( p->curr_weapon )
		{
			...
			case FREEZE_WEAPON:
				...
				if( (*kb) < 4 )
				{
					...
					if(s->xrepeat < 32 || p->ammo_amount[FREEZE_WEAPON] <= 0)
					{ *kb = 0; break; }
				}
				else
				{
