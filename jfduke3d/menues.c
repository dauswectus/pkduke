...
static int patchedspritessize = 0;
...
void dnNewGame(GameDesc *gamedesc) {
	...
	// PLK - g_LoadingLevel will hide the menus so the loading screen and intro animations can be seen
	//g_LoadingLevel = 1;
	//g_IsInPlayback = 0;
...
int  dnMenuModeSet() {
	return ps[myconnectindex].gm & MODE_MENU;
}

void dnSetBrightness(int brightness) {
	ud.brightness = brightness;
	setbrightness(ud.brightness>>2, &ps[myconnectindex].palette[0], 0);
}

int dnGetBrightness() {
	return ud.brightness;
}

void dnEnableSound(int enable) {
    if (FXDevice >= 0 && enable != SoundToggle) {
        SoundToggle = enable;
        if (SoundToggle == 0) {
            FX_StopAllSounds();
            clearsoundlocks();
...
void dnAssignKey(int function, int whichkey, int keycode) {
    KeyboardKeys[function][whichkey] = keycode;
    CONTROL_MapKey( function, KeyboardKeys[function][0], KeyboardKeys[function][1] );
}
...
void dnResetMouseKeyBindings(void) {
	int i;
    for (i = 0; i < 9; i++) {
        CONTROL_MapButton(-1, i, 0, controldevice_mouse);
        CONTROL_MapButton(-1, i, 1, controldevice_mouse);
    }
}
 */
extern char nofog;
...
void dnCaptureScreen() {
    screencapt = 1;
    displayrooms((short) myconnectindex,65536);
    screencapt = 0;
}
