...
void prelevel(char g)
{
    ...
    clearbufbyte(secretstosectormap, sizeof(secretstosectormap), 0L);
    ...
    for(i=0;i<numsectors;i++)
    {
        ...
        {
            secretstosectormap[ps[0].max_secret_rooms] = i;
...
void dnFoundSecret(short sectorindex)		// JEP COOP
{
	int i;
	for (i = 0; i < 16; i++)
	{
		if (secretstosectormap[i] == sectorindex)
		{
			break;
		}
	}
	...
	if (i < 16)
	{
		dnRecordSecret(ud.volume_number, ud.level_number, i);
	}
}
